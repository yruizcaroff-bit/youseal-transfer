...
async function createTransfer(input) {
  ...
  const transfer = {
    ...
    // Le transfert est efface des que chaque fichier a ete recupere une fois.
...
function publicView(transfer) {
  return {
    ...
    burnAfterReading: Boolean(transfer.burnAfterReading),
    downloads: transfer.downloads,
    maxDownloads: transfer.maxDownloads,
    totalSize: transfer.totalSize,
    files: transfer.files.map((f) => ({
      id: f.id, name: f.name, size: f.size, type: f.type, fetched: Boolean(f.fetched),
    })),
  };
}
