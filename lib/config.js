...
const config = {
  ...
  // Purge d'urgence : au-dela du seuil, les transferts les plus anciens sont
  // effaces jusqu'a liberer `pruneAmount`, meme s'ils n'ont pas expire.
  // Sans cela, le service refuse tout nouvel envoi une fois plein.
  pruneThreshold: int(process.env.PRUNE_THRESHOLD, 0),
  pruneAmount: int(process.env.PRUNE_AMOUNT, 0),
