...
/**
 * Les compteurs ne se reconstituent pas : ils ne derivent d'aucune autre
 * donnee, et les transferts qui les ont nourris ont expire depuis longtemps.
 * On garde donc systematiquement la version precedente, qui sert de recours si
 * le fichier principal est efface ou tronque.
 */
async function load() {
  for (const source of [statsFile(), `${statsFile()}.bak`]) {
    try {
      const lu = JSON.parse(await fsp.readFile(source, 'utf8'));
      if (Number.isFinite(lu.transfers)) {
        Object.assign(totals, lu);
        if (source.endsWith('.bak')) {
          console.warn('[compteurs] fichier principal illisible, reprise sur la sauvegarde');
          await flush();
        }
        return;
      }
...
async function flush() {
  ...
  const payload = JSON.stringify(totals, null, 2);
  try {
    const tmp = `${target}.${process.pid}.tmp`;
    await fsp.writeFile(tmp, payload);
    await fsp.rename(tmp, target);

    // Sauvegarde de la MEME generation, ecrite apres coup : conserver l'etat
    // precedent ne servirait a rien, il ne ferait que retarder la perte d'un
    // cran. Le passage par un fichier temporaire evite une copie tronquee.
    const bakTmp = `${target}.${process.pid}.bak.tmp`;
    await fsp.writeFile(bakTmp, payload);
    await fsp.rename(bakTmp, `${target}.bak`);
